...
class FusionHTTPServer(HTTPServer):
    """Single long-lived worker thread for reliable Fusion CustomEvent delivery."""
...
class SimpleMCPServer:
    ...
    async def handle_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not is_json_rpc_request(request):
    ...
    async def _execute_on_main_thread(self, handler_func, arguments: Dict[str, Any], request_id: Any, operation_type: str = "operation") -> Any:
        ...
        timeout = 120  # Larger ModelPlans can take time on Fusion's main thread.
...
class MCPHandler(BaseHTTPRequestHandler):
    ...
    def _is_trusted_local_request(self) -> bool:
        return is_loopback_host(self.headers.get("Host", "")) and is_allowed_origin(
    ...
    def do_POST(self):
        ...
        try:
            if not self._is_trusted_local_request():
                self.send_error(403, "Only trusted loopback requests are allowed")
                return
...
def start_mcp_server(
...
) -> Tuple[Optional[SimpleMCPServer], Optional[FusionHTTPServer], Optional[threading.Thread]]:
    """
    ...
    try:
        ...
        # Keep all fireCustomEvent calls on the same long-lived server thread.
        # Fusion API event delivery is unreliable across short-lived request threads,
        # and CAD writes should be serialized anyway.
        http_server = FusionHTTPServer(server_address, HandlerWithMCP)
