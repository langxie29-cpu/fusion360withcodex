"""Fusion AI Modeler add-in: a localhost-only, typed MCP modeling bridge."""
...
def run(context):
    ...
    try:
        ...
        if mcp:
            app.log(
                f"Fusion AI Modeler started successfully!\n\n"
                f"MCP server running on {HOST}:{PORT}\n"
                f"Typed tools: status, inspect, ModelPlan, screenshots."
            )
        else:
            if ui:
                ui.messageBox("Failed to start Fusion AI Modeler")
            if app:
                app.log("Failed to start Fusion AI Modeler")
    except Exception:
        app.log(f'Failed to start Fusion AI Modeler:\n{traceback.format_exc()}')
...
def stop(context):
    ...
    try:
        ...
        if stop_mcp_server(server, thread):
            if app:
                app.log("Fusion AI Modeler stopped successfully.")
        else:
            if app:
                app.log("Error stopping Fusion AI Modeler")

    except Exception:
        if app:
            app.log(f"Error stopping Fusion AI Modeler:\n{traceback.format_exc()}")
